...
void	tokenizer_less_handler(t_tokenizer *t)
{
	...
	if (t->curr_token)
	{
		if (t->curr_token->type == T_TT_WORD
			|| t->curr_token->type == T_TT_REDIR_IN)
			t->line[t->cursor] = 0;
	...
	{
		t->line[t->cursor] = 0;
		tokenizer_new_token(t, T_TT_REDIR_IN);
	}
}
